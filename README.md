<h1 align="center">hey, i'm luciano 👋</h1>

<p align="center">
  <img src="./assets/banner.svg" alt="yeahhhh" />
</p>

<p align="center">
  <img src="./assets/line.svg" width="100%" />
</p>

<h2 align="center">✨ about me</h2>

<ul align="center" style="list-style: none; padding: 0;">
  <li>🐛 currently in an entanglement with bugs</li>
  <li>☕ coffee > sleep</li>
  <li>💻 code ~> break ~> repeat</li>
  <li>🐍 writes Python but blames JS for everything</li>
  <li>🎧 soundtrack: lo-fi & existential dread</li>
</ul>

<p align="center">
  <img src="./assets/line.svg" width="100%" />
</p>

<h2 align="center">⚡ random stats</h2>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=kendroooo&theme=tokyonight&hide_border=true" height="150"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=kendroooo&layout=compact&theme=tokyonight&hide_border=true" height="150"/>
</p>

<p align="center">
  <img src="./assets/line.svg" width="100%" />
</p>

<p align="center">
  <b>wanna talk? just text me here on github lol</b>
</p>

<p align="center">
  <i>“it works on my machine” - kendroooo, probably</i>
</p>
